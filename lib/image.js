...
Img = (function() {
  ...
  function Img(request, logger) {
    ...
    this.cdnRoot = process.env.CDN_ROOT || ('https://s3-ap-southeast-1.amazonaws.com/' + this.bucket + '/');
  ...
  Img.prototype.filename = function() {
    return '' +
      this.uuid +
      (this._optionsString()) +
      '.' + (typeof this.options.ext == 'undefined' ? this.ext.toLowerCase() : this.options.ext);
  };

  // Build a path the image version on S3
  Img.prototype.s3Path = function() {
    return '' + this.prefix + this.env + '/' + this.filename();
  };
  ...
  Img.prototype._optionsString = function() {
    ...
    if (this.options.hasOwnProperty('bgcolor')) {
      str += '_bg' + this.options.bgcolor;
    }
    if (this.options.hasOwnProperty('ext')) {
      str += '_ext' + this.options.ext;
    }
    if (this.options.hasOwnProperty('quality')) {
      str += '_q' + this.options.quality;
    }
    if (this.options.hasOwnProperty('blur')) {
      str += '_blur' + this.options.blurRadius + 'x' + this.options.blurSigma;
    }
    if (this.options.hasOwnProperty('modulate')) {
      str += '_modulate' + this.options.modulateBrightness + 'x' + this.options.modulateSaturation + 'x' + this.options.modulateHue;
    }
    if (this.options.hasOwnProperty('watermark')) {
      str += '_watermark' + parseInt(this.options.watermark);
    }
