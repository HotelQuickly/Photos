...
module.exports = function(request){
  ...
  _.forEach(url.query, function(value, key){
	switch(key) {
		...
		// resize and crop the image: ?(f|fit)=[w],[h]
		case 'fit':
			opts = value.split(',');
			options.action = 'fit';
		...
		case 'blur':
			opts = value.split(',');
			options.blur  = true;
			options.blurRadius = sanitizeInput(opts[0]);
			options.blurSigma = sanitizeInput(opts[1]);
			break;

		case 'modulate':
			opts = value.split(',');
			options.modulate  = true;
			options.modulateBrightness = sanitizeInput(opts[0]);
			options.modulateSaturation = sanitizeInput(opts[1]);
			options.modulateHue = sanitizeInput(opts[2]);
			break;
		...
		case 'watermark':
			options.watermark = parseInt(sanitizeInput(value)) ? 1 : 0;
			break;
